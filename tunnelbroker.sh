...
network:
  ...
  tunnels:
    ...
    echo "OK"
        ...
        ;;
    5)
    read -r -p "Are You Sure to Unistall ipmart-ipv6? (${GREEN}yes${RESET}/${RED}no${RESET}): " answer
    if [ "$answer" == "yes" ]; then
    # اگر جواب yes بود، دستورات مورد نظر را اجرا کن
    echo "Oh ! Ok , No problem!"
    sudo unlink /usr/local/bin/ipmart-ipv6
    sudo rm -rf /root/ipmart-ipv6/
    exit 0
    # اینجا دستورات مورد نظر را قرار دهید
    else
